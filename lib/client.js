...
var SoloGameManager = require('./solo_game_manager.js').SoloGameManager;
...
function Client(config) {
  ...
  /**
   * Solo game manager
   * @type {SoloGameManager}
   */
  this._soloGameManager = null;

  /**
   * Flag indicating if the client is in solo mode
   * @type {boolean}
   */
   ...
   */
  this.reqPlaySolo = function (ruleName, callback) {
    this._isSoloMode = true;
    this._soloGameManager = new SoloGameManager();
    var rule = this.loadRule(ruleName);
    this._soloGameManager.init(rule);
    this.updateMatch(this._soloGameManager.match);
    this.updateRule(rule);
    this.updatePlayer(this._soloGameManager.match.host);
    this.updateOtherPlayer(this._soloGameManager.match.guest);
    this.resetBoard(this.match, this.rule);

    this.handleEventMatchStart(this.match);
    this.handleEventTurnStart(this.match);
  };
  ...
  this.reqRollDice = function (callback) {
    if (this._isSoloMode) {
      var result = this._soloGameManager.rollDice();
      this.handleMessage(comm.Message.EVENT_DICE_ROLL, result);
      this.updateMatch(this._soloGameManager.match);
      this.updateUI();
    } else {
  ...
  this.reqConfirmMoves = function (callback) {
    if (this._isSoloMode) {
      var result = this._soloGameManager.confirmMoves();
      this.handleMessage(result.event, result);
      this.updateMatch(this._soloGameManager.match);
      this.updateUI();
    } else {
  ...
  this.reqUndoMoves = function (callback) {
    if (this._isSoloMode) {
      var result = this._soloGameManager.undoMoves();
      if (result.result) {
        this.handleMessage(comm.Message.EVENT_UNDO_MOVES, result);
        this.updateMatch(this._soloGameManager.match);
        this.updateUI();
      } else {
        this.boardUI.notifyError(result.errorMessage);
      }
    } else {
  ...
  this.reqMove = function (piece, steps, callback) {
    if (this._isSoloMode) {
      var result = this._soloGameManager.movePiece(piece, steps);
      if (result.result) {
        this.handleMessage(comm.Message.EVENT_PIECE_MOVE, result);
        this.updateMatch(this._soloGameManager.match);
        this.updateUI();
      } else {
        this.boardUI.notifyError(result.errorMessage);
